package ru.levelup.client.api.utils.lib;
...
public class GaussianFilter {
    ...
    static {
        GaussianFilter.ZERO_EDGES = 0;
        GaussianFilter.CLAMP_EDGES = 1;
        GaussianFilter.WRAP_EDGES = 2;
    }
    protected float radius;
    private Kernel kernel = makeKernel(radius);
    ...
    public BufferedImage filter(final BufferedImage src, BufferedImage dst) {
        ...
        if (this.radius > 0.0f) {
            convolveAndTranspose(this.kernel, inPixels, outPixels, width, height, this.alpha,
                    this.alpha && this.premultiplyAlpha, false, GaussianFilter.CLAMP_EDGES);
            convolveAndTranspose(this.kernel, outPixels, inPixels, height, width, this.alpha, false,
                    this.alpha && this.premultiplyAlpha, GaussianFilter.CLAMP_EDGES);
        }
