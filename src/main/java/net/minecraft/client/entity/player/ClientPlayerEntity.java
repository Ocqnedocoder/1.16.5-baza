...
public class ClientPlayerEntity extends AbstractClientPlayerEntity
{
    ...
    {
        EventRotation rotation = (EventRotation) new EventRotation(this.rotationYaw, this.rotationPitch, this.getPosX(), this.getPosY(), this.getPosZ()).call();
        ...
        if (this.isCurrentViewEntity())
        {
            ...
            if (this.isPassenger())
            {
                ...
                this.connection.sendPacket(new CPlayerPacket.PositionRotationPacket(vector3d.x, -999.0D, vector3d.z, rotation.getYaw(), rotation.getPitch(), this.onGround));
                flag1 = false;
            }
            else if (flag1 && flag2)
            {
                this.connection.sendPacket(new CPlayerPacket.PositionRotationPacket(rotation.getX(), rotation.getY(), rotation.getZ(), rotation.getYaw(), rotation.getPitch(), this.onGround));
            }
            else if (flag1)
            {
                this.connection.sendPacket(new CPlayerPacket.PositionPacket(rotation.getX(), rotation.getY(), rotation.getZ(), this.onGround));
            }
            else if (flag2)
            {
                this.connection.sendPacket(new CPlayerPacket.RotationPacket( rotation.getYaw(), rotation.getPitch(), this.onGround));
            }
            ...
            if (flag1)
            {
                this.lastReportedPosX = rotation.getX();
                this.lastReportedPosY = rotation.getY();
                this.lastReportedPosZ = rotation.getZ();
                this.positionUpdateTicks = 0;
            }

            if (flag2)
            {
                this.lastReportedYaw = rotation.getYaw();
                this.lastReportedPitch = rotation.getPitch();
            }
