        ...
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, entityIn, this, partialTicks, matrixStackIn, bufferIn, packedLightIn))
        {
            ...
            if (rendertype != null)
            {
                ...
                if (flag)
                {
                    ...
                    {
                        var hitColorModule = Client.getInstance().getManager().getModule("HitColor");
                        if (hitColorModule.getToggled()) {
                            var settingManager = Client.getInstance().getSettingManager();
                            var redg = settingManager.getFloatSetting(hitColorModule, "Red");
                            var greeng = settingManager.getFloatSetting(hitColorModule, "Green");
                            var blueg = settingManager.getFloatSetting(hitColorModule, "Blue");
                            var alphags = settingManager.getFloatSetting(hitColorModule, "Alpha");
                            float alphag = (alphags.current) / 255.0f;
                            float redgs = (redg.current) / 255.0f;
                            float greengs = (greeng.current) / 255.0f;
                            float bluegs = (blueg.current) / 255.0f;
                            Shaders.setEntityColor(redgs, greengs, bluegs, alphag);
                        } else {
