...
import ru.levelup.client.api.module.impl.visuals.SwingAnimations;
    ...
    private void renderItemInFirstPerson(AbstractClientPlayerEntity player, float partialTicks, float pitch, Hand handIn, float swingProgress, ItemStack stack, float equippedProgress, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn)
    {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand(handIn))
        {
            ...
            else
            {
                ...
                else if (Client.getInstance().getManager().getModule("SwingAnimation").getToggled()) {

                    assert this.mc.player != null;
                    float swingprogress = this.mc.player.getSwingProgress(partialTicks);
                    var mode = Client.getInstance().getSettingManager().getModeSettings(
                            Client.getInstance().getManager().getModule("SwingAnimation"),
                                    "Animation");
                    if (Client.getInstance().getManager().getModule("SwingAnimation").getToggled()) {
                        var setting = Client.getInstance().getSettingManager().getAimSetting(
                                Client.getInstance().getManager().getModule("SwingAnimation"), "Left Hand & Right Hand");
                        var setting2 = Client.getInstance().getSettingManager().getFloatSetting(
                                Client.getInstance().getManager().getModule("SwingAnimation"), "Size Hand");
                        if (Client.getInstance().getManager().getModule("SwingAnimation").getToggled()) {
                            if (flag3) {
                                matrixStackIn.translate(setting.current3, -setting.current4, -setting2.current);
                            } else {
                                matrixStackIn.translate(setting.current1, -setting.current2, -setting2.current);
                            }
                        }
                        if (handside != (this.mc.gameSettings.mainHand.equals((Object) HandSide.LEFT) ? HandSide.RIGHT : HandSide.LEFT)) {
                            if (mode.is(SwingAnimations.Swings.DEFAULT)) {
                                float sin = MathHelper.sin(MathHelper.sqrt(swingprogress) * 3.0f);
                                matrixStackIn.rotate(Vector3f.YP.rotationDegrees(sin * 20.0f));
                                matrixStackIn.rotate(Vector3f.XP.rotationDegrees(sin * -90.0f));
                                matrixStackIn.scale(0.6f, 0.6f, 0.6f);
                            }
                            if (mode.is(SwingAnimations.Swings.BLOCK)) {
                                float sin = MathHelper.sin(MathHelper.sqrt(swingprogress) * 3.0f);
                                matrixStackIn.rotate(Vector3f.YP.rotationDegrees(-130));
                                matrixStackIn.rotate(Vector3f.XP.rotationDegrees(20));
                                matrixStackIn.rotate(Vector3f.XP.rotationDegrees(sin * 90.0f));
                                matrixStackIn.scale(0.6f, 0.6f, 0.6f);
                            }
                            if (mode.is(SwingAnimations.Swings.SPIN)){
                                float smooth = swingprogress * 4;
                                float sin = MathHelper.sin(MathHelper.sqrt(swingprogress) * 3.0f);
                                matrixStackIn.rotate(Vector3f.YP.rotationDegrees(-180));
                                matrixStackIn.rotate(Vector3f.XP.rotationDegrees(smooth * -100.0f));
                                matrixStackIn.rotate(Vector3f.XP.rotationDegrees(sin * -90.0f));
                                matrixStackIn.scale(0.6f, 0.6f, 0.6f);
                            }
                        }else {
