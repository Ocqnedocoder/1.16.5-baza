...
public class ResourceLoadProgressGui extends LoadingGui
{
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks)
    {
        ...
        DrawHelper.drawRound(0, 0, Minecraft.getInstance().getMainWindow().getScaledWidth(), Minecraft.getInstance().getMainWindow().getScaledHeight(), 0, ColorUtil.getColor(20, 255));


        ResourceLocation resourceLocation = new ResourceLocation("kuzureta/loadscreen/" + currentImageIndex + ".png");
        int imageWidth = 16;
        int imageHeight = 16;
        int x = (Minecraft.getInstance().getMainWindow().getScaledWidth() / 2) - (imageWidth / 2);
        int y = (Minecraft.getInstance().getMainWindow().getScaledHeight() / 2) - (imageHeight / 2);

        int color = 0xFFFFFFFF;
        ImageUtils.drawImage(resourceLocation, x, y, imageWidth, imageHeight, color);
