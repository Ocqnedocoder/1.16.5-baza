...
public class IngameGui extends AbstractGui
{
    ...
    public void renderIngameGui(MatrixStack matrixStack, float partialTicks)
    {
        ...
        var setts = Client.getInstance().getSettingManager().getBooleanSetting(
                Client.getInstance().getManager().getModule("Interface"), "Potion Info");
        if (setts.get() != BooleanSetting.ToggleState.ON) {
        ...
        if (!this.mc.gameSettings.hideGUI){
            ...
            for (Module module : Client.getInstance().getManager().getModules()){
                if (module.getToggled()){
                    DrawHelper.drawBlur(10, ()-> module.renderShader());
                }
            }

            EventScreen eventScreen = (EventScreen) new EventScreen(matrixStack).call();
